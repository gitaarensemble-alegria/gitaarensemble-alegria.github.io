$(function () {
  ...
  $("#event-submission-form input[data-type=file], #event-submission-form input[data-type=image]").on("change",function(){
    $(this).data("uploadedfiles",[]);
  });
  var apikey = "5a95abb5696ea8f017645981"; // TODO: INSERT YOUR CORS API KEY HERE OR add formapikey to settings
  ...
  var ajaxSettings = {
    "async": true,
    "crossDomain": true,
    "url": "https://alegriaforms-cd35.restdb.io/rest/event-submission",
    "method": "POST",
    "headers": {
      "x-apikey": apikey,
  ...
  var ajaxSettingsAttachments = {
     "async": true,
     "url": "https://alegriaforms-cd35.restdb.io/media",
     "method": "POST",
     "contentType": false,
     "headers": {
       "x-apikey": apikey
  ...
  function postForm() {
    ...
    // clear errors
    $("#event-submission-form .has-error").removeClass("has-error");
    $("#event-submission-form .help-block").remove();
 ...
 // get the form data
    var formObj = $("#event-submission-form").serializeObject();

    // get attachments from inputs
    var attachments = [];

    $("#event-submission-form input[data-type=file], #event-submission-form input[data-type=image]").each(function(input){
      ...
      .done(function(){
       ...
       ajaxSettings.data = JSON.stringify(formObj);
        ...
        .done(function (response) {
          ...
          $("#event-submission-form").replaceWith("<div class='thank-you'>"+successMessage+"</div>");
        })
